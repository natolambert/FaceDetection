...
# Sound library
from playsound import playsound as playSoundToggle

# Our Functions
from clientdefs import AttenthiaClient, get_jacobs_lat_lon			# Web Client commands
...
def main(user):
		...
		if (tl - t_state) > change_time:
			...
			if state != state_temp:
				...
				if state == 'clear':
					...
					if WebToggle:
						lat, lon = get_jacobs_lat_lon()
						cli.log(distraction_type, datetime.datetime.now(), lat, lon)
				elif state == 'distract':
					distraction_type = 1
					# Logs data to server args['user']
					if WebToggle:
				...
				# Driver not looking at camera
					...
					if SerialToggle:
						...
						print('distact')
					if SoundToggle:
						playSoundToggle('beep-01a.mp3',block=False)
			...
			# only plays distact SoundToggle if the state repeated as eyes closed
				...
				if SerialToggle:
					...
					print('tired')
				if SoundToggle:
					playSoundToggle('beep-02a.mp3',block=False)
				# Logs data to server args['user']
				distraction_type=2
				if WebToggle:
					lat, lon = get_jacobs_lat_lon()
	...
	print "You can view the trip info at {}".format(cli.get_trip_url())
...
if __name__=='__main__':
	while True:
		username = raw_input("\n\nWhat is your username? Or q to quit >>>")
		if username == 'q':
			conf = raw_input("\n\nAre you sure? Y/N >>>")
			if conf == 'Y':
				break
